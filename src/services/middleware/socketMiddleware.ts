...
import { TAccessSchemaWithToken } from '../api';
...
export const socketMiddlewareFabric = (
  ...
  return (
    ...
    return next =>(action: PayloadAction<WsGeneralActionPayload, WsActionTypes[keyof WsActionTypes]>) => {
      ...
      switch (type) {
        case wsActionTypes.wsOpenConnection: {
          ...
          const { accessToken } = (auth ?? {}) as TAccessSchemaWithToken;
